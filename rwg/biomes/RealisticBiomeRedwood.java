...
public class RealisticBiomeRedwood extends BiomeGenBase implements RealisticBiome
{
	public RealisticBiomeRedwood(int id)
	{
		super(id);
	}
	...
	public void rDecorate(World world, Random rand, int chunkX, int chunkY, PerlinNoise perlin, float strength) 
	{
		...
		if(rand.nextInt(l > 0.2f ? (int)(5f / strength) : 1) == 0)
		{
		...
		for (int b1 = l > 0.2f ? 1 : (int)(4f * strength); b1 > -1; b1--)
		{
			int j6 = chunkX + rand.nextInt(16) + 8;
			int k10 = chunkY + rand.nextInt(16) + 8;
			int z52 = world.getHeightValue(j6, k10);
		...
		for (int b1 = 0; b1 < 7f * strength; b1++)
		{
			int j6 = chunkX + rand.nextInt(16) + 8;
			int k10 = chunkY + rand.nextInt(16) + 8;
			int z52 = world.getHeightValue(j6, k10);
